...
import { ArticleMetaProps } from '@/types/GlobalTypes'
        ...
        <div className="grid grid-cols-3 gap-4">
            ...
            {getMultiselect(page.properties.tags.multi_select).map(
              (tag: string, index: number) => (
                <Link key={index} href={`/tags/${tag}`} className='text-gray-700 no-underline border-solid border-gray-700 opacity-70 mr-3'>
                  <span>{`#${tag}`}</span>
                </Link>
            ))}
            </div>
